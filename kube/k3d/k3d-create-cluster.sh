...
k3d cluster create ${APPNAME} --config ${SCRIPT_DIR}/k3d-config.yml \
  --volume ${SCRIPT_DIR}/../../logbook:/ijmacd/logbook@all
...
kubectl create secret generic logbook-mariadb-auth \
  -n ${APPNAME} \
  --from-literal=mariadb-root-password=$(openssl rand -base64 18) \
  --from-literal=mariadb-password=$(openssl rand -base64 18)
